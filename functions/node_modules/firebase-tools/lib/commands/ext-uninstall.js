...
const modsApi = require("../extensions/modsApi");
const modsHelper_1 = require("../extensions/modsHelper");
        ...
        const resourcesMessage = _.get(instance, "configuration.source.spec.resources", []).length
            ? "This will delete the following resources \n" +
                instance.configuration.source.spec.resources
                    ...
                    .join("")
            : "";
        const modDeletionMessage = `You are about to uninstall extension ${clc.bold(instanceId)} from project ${clc.bold(projectId)}.\n${resourcesMessage}Are you sure?`;
        ...
        if (confirmedServiceAccountDeletion || options.force) {
            ...
            if (_.get(saDeletionRes, "body.error")) {
                if (_.get(saDeletionRes, "body.error.code") === 404) {
                    spinner.succeed(` ${clc.green.bold(modsHelper_1.logPrefix)}: service account ${clc.bold(instance.serviceAccountEmail)} was previously deleted.`);
                }
                else {
                ...
                spinner.succeed(` ${clc.green.bold(modsHelper_1.logPrefix)}: deleted service account ${clc.bold(instance.serviceAccountEmail)}`);
            }
        }
    }
    catch (err) {
